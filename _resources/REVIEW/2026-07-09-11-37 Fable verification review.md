# Fable verification review — commit 08707f7 "Modernize Floor Whisper and add architect sheet export"
...
- `__root.tsx` meta/title, `TopBar`, `AGENTS.md`, `package.json` name: all correctly
  "Floor Whisper — Residale", French copy. Confirmed.
- **FAIL found & fixed:** the "Dossier PDF" export footer still stamped **"Plana Studio"**
  ...
  implementation report's "rebranded visible metadata" claim). Now "Floor Whisper — Residale".
...
- **Bugs found & fixed:** (1) *medium* — elevations auto-scale to a different scale than the
...
and held up (file lists, removals, backup branch `floor-whisper-backup` + tag
...
- `https://config.residale.com` returned HTTP 200 with title "Floor Whisper — Residale";
